    ...
    public static async createTestData(ctx: BaseContext) {
        try {
            ...
            await getConnection()
                ...
                .into(User)
                .values([
                    { givenName: 'Julian', familyName: 'Inwood', email: 'julian@inwood.com', password: 'pass123' },
                    { givenName: 'Vanessa', familyName: 'Humphreys', email: 'chumpers@gmail.com', password: 'pass123' }
                ])
                .execute();
            ...
            const vehicle = await getConnection()
                ...
                .into(Vehicle)
                .values([
                    { manufacturer: 'Mercedes-Benz', model: 'C320', engineSize: 3200, registration: 'ABC123' }
                ]).execute();
            ...
            const historyItem = await getConnection()
                ...
                .into(HistoryItem)
                .values([
                    { type: 1, description: 'Oil service', media: ['http://s3-media3.fl.yelpcdn.com/bphoto/Gx7GQWqkBfRI1mYMF4n4dg/o.jpg'] }
                ]).execute();
