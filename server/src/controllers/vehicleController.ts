...
export default class VehicleController { 
    ...
    public static async getVehicles (ctx: BaseContext) {
        ...
        const vehicleRepository: Repository<Vehicle> = getManager().getRepository(Vehicle);
    ...
    public static async createVehicle (ctx: BaseContext) { 
        ...
        const validationErrors: ValidationError[] = await validate(newVehicle, { skipMissingProperties: true });
        if (validationErrors.length > 0) {
            ctx.status = 400;
            ctx.body = validationErrors;
        } else if (await vehicleRepository.findOne({ registration: newVehicle.registration })) {
            ctx.status = 400;
